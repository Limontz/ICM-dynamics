function average_radial_perturb(dens, d_prof, temp, t_prof, vx, vx_prof, vy, vy_prof, vz, vz_prof, radial_bin, xc, yc, zc, rmax, thickness)
          ...
          dens_shell_mean2 = zeros(radial_bin)
    ...
    @inbounds     for j in 1:length(dens[1,:])
                  ...
                  @fastmath r=Int(floor(20*(1/0.1)*kpc*sqrt((j-yc -0.5)^2. +(i- xc -0.5)^2.)/rmax))
                            ...
                            if(ra == l)
                               ...
                               append!(v_shell,sqrt(vx[i,j]^2+vy[i,j]^2+vz[i,j]^2))
                ...
                dens_shell_mean2[l] = std(dens_shell.^2)/d_prof[l]^2

                #println(std(dens_shell))
          ...
          return(dens_shell_mean, dens_shell_mean2, temp_shell_mean, v_shell_mean, v_shell_mean2)
