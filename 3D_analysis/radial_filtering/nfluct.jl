...
function nfluct(Npoint, d_conv, radial_bin, xc, yc, zc, r_max )
         ...
         @inbounds for k in 1:length(d_conv[1,1,:])
         @inbounds     for j in 1:length(d_conv[1,1,:])
         @inbounds @simd    for i in 1:length(d_conv[1,1,:])
                      ...
                      @fastmath r=Int(floor(200*kpc*sqrt((k -zc -0.5)^2. +(j-yc -0.5)^2. +(i- xc -0.5)^2.)/r_max))
                                ...
                                d_r[ra,n_fluct[ra]]=d_conv[i,j,k]
                            end
                        end
                   end
